...
@pytest.fixture
def authed_client(settings: Settings, verifier: ClerkVerifier) -> TestClient:
...
@respx.mock
def test_etch_state_forwards_the_bearer_token(settings: Settings) -> None:
    route = respx.get("http://ledboard.test/etch").respond(200, json=STATE)
    result = etch_state(settings, "tok")
...
def test_etch_move_posts_the_nudge(settings: Settings) -> None:
    ...
    result = etch_move(settings, "tok", 4, 0)
...
def test_etch_clear_posts_empty(settings: Settings) -> None:
    ...
    assert etch_clear(settings, "tok").body == {"cleared": True}
...
@pytest.mark.parametrize("method,path", [("GET", "/api/etch"), ("POST", "/api/etch/move")])
def test_etch_requires_a_token(authed_client: TestClient, method: str, path: str) -> None:
    payload = {} if method == "GET" else {"dx": 1}
    response = authed_client.request(method, path, json=payload)
    assert response.status_code == 401


def test_etch_clear_requires_a_token(authed_client: TestClient) -> None:
    assert authed_client.post("/api/etch/clear").status_code == 401


def test_get_etch_returns_the_pi_state(
    authed_client: TestClient, auth: dict[str, str], board: respx.MockRouter
) -> None:
    board.get("http://ledboard.test/etch").respond(200, json=STATE)
    response = authed_client.get("/api/etch", headers=auth)
    assert response.status_code == 200, response.text
    assert response.json() == STATE


def test_post_move_returns_the_new_cursor(
    authed_client: TestClient, auth: dict[str, str], board: respx.MockRouter
) -> None:
    route = board.post("http://ledboard.test/etch/move").respond(200, json={"x": 68, "y": 16})
    response = authed_client.post("/api/etch/move", json={"dx": 4, "dy": 0}, headers=auth)
    assert response.status_code == 200, response.text
    assert response.json() == {"x": 68, "y": 16}
    assert route.calls.last.request.read() == b'{"dx":4,"dy":0}'


def test_post_clear_shakes(
    authed_client: TestClient, auth: dict[str, str], board: respx.MockRouter
) -> None:
    board.post("http://ledboard.test/etch/clear").respond(
        200, json={"cleared": True, "x": 1, "y": 2}
    )
    response = authed_client.post("/api/etch/clear", headers=auth)
    assert response.status_code == 200, response.text
...
def test_post_move_rejects_bad_nudges(
    authed_client: TestClient, auth: dict[str, str], board: respx.MockRouter, body: dict
) -> None:
    response = authed_client.post("/api/etch/move", json=body, headers=auth)
    assert response.status_code == 422, f"{body!r} is not a knob nudge"
    assert not board.calls, "invalid input never reaches the Pi"


def test_unreachable_pi_is_502(
    authed_client: TestClient, auth: dict[str, str], board: respx.MockRouter
) -> None:
...
def test_disabled_etch_app_passes_through(
    authed_client: TestClient, auth: dict[str, str], board: respx.MockRouter
) -> None:
